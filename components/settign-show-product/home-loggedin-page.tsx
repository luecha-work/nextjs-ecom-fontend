...
interface HomeLoggedinPageForm {
  id: string;
  isLoggedIn: boolean;
...
const HomeLoggedinPage: NextPage = () => {
      ...
      preferencesShowProductsService
        ...
        .then((res) => {
          if (res?.status_code === "200") {
            setTextModal("ตั้งค่าหน้าสินค้าแนะนำสำเร็จ");
            setColorIconModal("green");

            setOpen(true);
          } else {
            setColorIconModal("red");
            setTextModal("ตั้งค่าหน้าสินค้าแนะนำล้มเหลว");
          }
        });
    }
  };
  ...
  useEffect(() => {
    ...
    preferencesShowProductsService.getIntroductionPageSetting().then((res) => {
